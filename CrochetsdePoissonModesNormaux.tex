...
\begin{flalign*}
&\{L_n,L_m\}=\frac{1}{4}\sum_{k=-\infty}^{k=+\infty}\sum_{l=-\infty}^{l=+\infty}\{\vec{a}_{-k} \cdot \vec{a}_{n+k},\vec{a}_{-l} \cdot \vec{a}_{m+l}\}=\frac{1}{4}\sum_{k=-\infty}^{k=+\infty}\sum_{l=-\infty}^{l=+\infty}\sum_{i=1}^{D-1}\sum_{j=1}^{D-1}\{a^i_{-k}a^i_{n+k},a^j_{-l}a^j_{m+l}\}\\
...
Donc
\begin{flalign*}
\{L_n,L_m\}=\frac{1}{4}\sum_{k=-\infty}^{k=+\infty}\sum_{l=-\infty}^{l=+\infty}\sum_{i=1}^{D-1}\sum_{j=1}^{D-1}& a^i_{-k}(-i(n+k)\delta_{n+k,l}\delta^{ij})a^j_{m+l}+a^i_{-k}(-i(n+k)\delta_{n+k,-m-l}\delta^{ij})a^j_{-l}\\
&+a^i_{n+k}(-i(-k)\delta_{-k,l}\delta^{ij})a^j_{m+l}+a^i_{n+k}(-i(-k)\delta_{-k,-m-l}\delta^{ij})a^j_{-l}
\end{flalign*}
\begin{flalign*}
&=\frac{1}{4}\sum_{k=-\infty}^{k=+\infty}\sum_{i=1}^{D-1} \left[ -i(n+k)a^i_{-k}a^i_{m+n+k}-i(n+k)a^i_{-k}a^i_{m+n+k}+ika^i_{n+k}a^i_{m-k}+ika^i_{n+k}a^i_{m-k}\right]\\
&=\frac{1}{2}\sum_{k=-\infty}^{k=+\infty}\sum_{i=1}^{D-1} -i(n+k)a^i_{-k}a^i_{m+n+k}+\frac{1}{2}\sum_{k=-\infty}^{k=+\infty}\sum_{i=1}^{D-1}ika^i_{n+k}a^i_{m-k}\\
&=\frac{1}{2}\sum_{k=-\infty}^{k=+\infty}\sum_{i=1}^{D-1} -i(n+k)a^i_{-k}a^i_{m+n+k}+\frac{1}{2}\sum_{k'=-\infty}^{k=+\infty}\sum_{i=1}^{D-1}i(k'+m)a^i_{n+m+k'}a^i_{-k'}\\
&=\frac{1}{2}\sum_{k=-\infty}^{k=+\infty}\sum_{i=1}^{D-1} i(m-n)a^i_{-k}a^i_{m+n+k}=i(m-n)L_{m+n}
\end{flalign*}
\begin{flalign*}
\{L_n,a^j_m\}&=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{i=1}^{D-1}\{a^i_{-k}a^i_{n+k},a^j_m\}=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{i=1}^{D-1}\left[ a^i_{-k}\{a^i_{n+k},a^j_m\}+\{a^i_{-k},a^j_m\}a^i_{n+k}\right]\\
&=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{i=1}^{D-1}\left[ a^i_{-k}(-i(n+k)\delta_{n+k,-m}\delta_{i,j})+(-i(-k)\delta_{-k,-m}\delta_{i,j}a^i_{n+k}\right]\\
&=\frac{1}{2}\left[ima^j_{m+n}+ima^j_{n+m}\right]=ima^j_{m+n}
\end{flalign*}
\begin{flalign*}
\{q_0^i,L_{m}\}&=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{j=1}^{D-1}\{q_0^i,a_{-k}^j \cdot a_{m+k}^{j}\}=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{j=1}^{D-1}\left[\{q_0^i,a_{-k}^j\}a_{m+k}^{j}+a_{-k}^j\{q_0^i,a_{m+k}^{j}\}\right]\\
&=\frac{1}{2}\sum_{k=-\infty}^{+\infty}\sum_{j=1}^{D-1}\left[\sqrt{2}\delta_{0,-k}\delta{ij}a_{n+k}^{j}+a_{-k}^j\sqrt{2}\delta_{0,m+k}\delta^{ij}\right]=\sqrt{2}a_{m}^i
\end{flalign*}
