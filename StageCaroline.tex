...
Pour que le problème soit physique, on impose que chaque point de cette surface se déplace à une vitesse $v\leq c$, c'est-à-dire qu'à chaque point de la surface, il existe un déplacement infinitésimal le long de la surface de type-temps ou de type-lumière. On dit que cette surface est de type-temps.
\subsection{Paramétrisation générale}
\textit{Partie à retravailler}
...
L'aire de la surface $\Sigma$ s'écrit alors: $$A=\int d^n\xi\sqrt{-\gamma}=\sqrt{-det(\partial_\alpha x\partial_\beta x)}$$
...
Soit un ensemble de champs différentiables $\phi$ défini sur tout l'espace-temps. L'action est une intégrale sur l'espace et le temps:
...
Par le théorème de Noether, ces deux transformations vont donner lieu à une densité de courant conservé.
\textit{retravailler la suite}
...
\textit{Ce qui suit est faux,comprendre et réécrire}\\
...
$q_-(\tau)$ ne dépend pas de $\sigma$ donc le premier terme de l'intégrale donne 0. Pour le second terme, on utilise la relation (2.7.3): $\frac{\pi}{\CP}\vecx'\vecpt=x'_-$.
On réécrit donc: 
$$ q_-(\tau)=\frac{1}{\pi}\int_0^\pi d\sigma\left[ \int_0^\pi d\sigma'\left( \frac{\sigma'}{\pi}-\theta(\sigma'-\sigma)\right) \frac{\partial x_-}{\partial \sigma'}\right]$$
$$=\frac{1}{\pi^2}\int_0^\pi d\sigma\left[\sigma'.x_-(\sigma',\tau)\right]^\pi_0 - \frac{1}{\pi^2}\int_0^\pi d\sigma\int_0^\pi d\sigma'\frac{\partial x_-}{\partial \sigma'}(\sigma',\tau) - \frac{1}{\pi}\int_0^\pi d\sigma\int_\sigma^\pi d\sigma'\frac{\partial x_-}{\partial \sigma'}(\sigma',\tau)$$
$$=\frac{1}{\pi^2}\int_0^\pi d\sigma \pi.x_-(\pi,\tau)-\frac{1}{\pi^2}\int_0^\pi d\sigma \left( x_-(\pi,\tau)-x_-(0,\tau)\right) - \frac{1}{\pi}\int_0^\pi d\sigma\left( x_-(\pi,\tau)-x_-(\sigma,\tau)\right) $$
Les quatres premiers termes donnent une intégrale nulle car ils ne dépendent pas de $\sigma$. Le dernier terme nous redonne:
$$q_-(\tau)=\frac{1}{\pi}\int_0^\pi d\sigma x_-(\sigma,\tau)$$
Cette expression pour $x_-(\sigma,\tau)$ est bien cohérente.\\
Nous pouvons donc exprimer toutes les composantes de $x$ et $\pt$ à partir de:
...
\textbf{Attention} la dérivée partielle par rapport à $\tau$ ci-dessus doit se comprendre comme une dérivée explicite, qui sera toujours nulle ici puisqu'aucune de nos expressions ne dépend explicitement de $\tau$.\\
\paragraph{$\vecpt$:}
$$\left( \ddot{\xmu}-x^{\mu''} \right)=0 $$
Or $\dot{\xmu}=2\pi \pt^\mu$,
...
A présent, on doit exprimer l'Hamiltonien du système. On est tenté pour ce faire de retourner à l'expression du Lagrangien du système et de trouver l'Hamiltonien par transformée de Legendre. Cependant cette méthode n'est plus valable maintenant qu'on a fixé une partie des variables par les contraintes (si on le fait, on obtient des contradictions comme $H=0$). Mais on sait que l'Hamiltonien décrit l'évolution temporelle (dans notre cas selon $\tau$) du système. Or, par la relation (2.7.1), on sait que $x_+=2\CP_+.\tau$. Il est donc naturel d'écrire que $H=2\CP_+\CP_-$ où $\CP_-$ est la variable "conjuguée?" de $\CP_+$(et le générateur des translations infinitésimales dans la direction $x_+$).
Par l'équation (22) et la définition de $\CP_-$, on obtient: $$H=\pi\int_0^\pi\left( \vecpt^2 +\frac{\vecx^{'2}}{(2\pi)^2}\right)=\dot{q_-}\CP_+ $$
On est à présent prêts pour calculer les crochets de Poisson:
