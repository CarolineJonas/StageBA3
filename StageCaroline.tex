...
La densité de courant associée au trnsformations de Lorentz est:
$M_{i}^{\mu\nu}\equiv x^{\mu}P^{\nu}_{i}-x^{\nu}P^{\mu}_{i}$\\
...
n.P_\tau=\frac{n.\mathcal{P}}{\pi}
\end{align}
Autrement dit, $\xmu$ et $\pt^\mu$ ne sont donc pas indépendantes l'une de l'autre. Sous contraintes, on a deux méthodes pour obtenir le formalisme Hamiltonien:
\begin{enumerate}
...
D'autre part, on sait que $\CP_-$ génére les translations infinitésimales dans la direction de $x_+$: $$\frac{\partial}{\partial \CP_-}\leftrightarrow x_+$$
...
\begin{equation}
	\left\lbrace
    \begin{aligned}
		\dot{q}^i=\{q^i,\mathcal{H}\}=\frac{\partial \mathcal{H}}{\partial p^i}\\
		\dot{p}^i=\{p^i,\mathcal{H}\}=-\frac{\partial \mathcal{H}}{\partial q^i}\\
     \end{aligned}
     \right.
\end{equation}
où $$\mathcal{H}= \pi\left( \vecpt^2 +\frac{\vecx^{'2}}{(2\pi)^2}\right)$$ est la densité de Hamiltonien. Montrons que ces équations sont équivalentes pour $q^i=x^i(\sigma)$, $p^i=\pt^i(\sigma)$:
\begin{align*}
	&\frac{\partial\mathcal{H}}{\partial p^i}=\frac{\partial\mathcal{H}}{\partial \pt^i}=2\pi*\pt^i\equiv \dot{x}^i=\dot{q}^i&\\
	&\frac{\partial\mathcal{H}}{\partial q^i}=\frac{\partial\mathcal{H}}{\partial x^i}=\pi\sum_{j=1}^{D-1} \frac{2x^{'j}}{(2\pi)^2}\frac{\partial x^{'j}}{\partial x^i}=\sum_{j=1}^{D-1}\frac{x^{'j}}{2\pi}*\frac{x^{''j}}{x^{'i}}\delta^{ij}=\frac{1}{2\pi}x^{''i}=\dot{\pt}^i=\dot{p}^i&
\end{align*}
...
Pour exprimer les $a_n^-$, il faut travailler les expressions un peu plus:
\begin{flalign}
\sqrt{2}i\sum_{n=-\infty;n\neq 0}^{+\infty}\frac{a_n^-}{n}\cos(n\sigma)e^{-in\tau}&=\frac{\sqrt{2}\pi}{a_0^+}\int_0^\pi d\sigma'\left( \frac{\sigma'}{\pi}-\theta(\sigma'-\sigma)\right) \vecx'\vecpt\\
&\equiv f(\sigma,\tau)
\end{flalign}
Alors
\begin{flalign}
...
Donc
\begin{equation}
\vec{x}^{ '}\vec{\pt}=\frac{-i}{\pi}\sum_{n=-\infty;n\neq 0}^{+\infty}\sum_{k=-\infty}^{+\infty}\vec{a}_n\vec{a}_k\sin(n\sigma')\cos(k\sigma')e^{-in\tau}e^{-ik\tau}
\end{equation}
Ainsi,
\begin{flalign}
&\int_0^\pi f(\sigma,\tau)\cos(m\sigma)d\sigma=\\
&\frac{-\sqrt{2}i}{a_0^+}\sum_{n=-\infty;n\neq 0}^{+\infty}\sum_{k=-\infty}^{+\infty}\vec{a}_n\vec{a}_ke^{-i(n+k)\tau}\int_0^\pi d\sigma\cos(m\sigma) \left[\int_0^\pi d\sigma'\left( \frac{\sigma'}{\pi}-\theta(\sigma'-\sigma)\right) \sin(n\sigma')\cos(k\sigma')\right]
\end{flalign}
\begin{flalign}
